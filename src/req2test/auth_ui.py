...
def render_auth_html(mode: str) -> str:
    ...
    action = "CREATE ACCOUNT" if register else "LOGIN"
    action_zh = "创建账户" if register else "登录"
    alternate_href = "/login" if register else "/register"
    alternate = "BACK TO LOGIN / 返回登录" if register else "CREATE ACCOUNT / 注册账户"
    confirm = (
        '<label>CONFIRM PASSWORD <span>确认密码</span></label>'
